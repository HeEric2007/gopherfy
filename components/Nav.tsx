...
export function Nav() {
  ...
  const renderLink = (link: (typeof nav.links)[number]) => {
    const isAnchor = link.href.includes("#");
    const className = "transition hover:text-neutral-950";
    return isAnchor ? (
  ...
  return (
    <header className="w-full">
      <div className="mx-auto max-w-6xl px-4 py-4 sm:px-8 sm:py-6">
        ...
        <ul className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm font-medium text-neutral-700 sm:hidden">
          {nav.links.map((link) => (
            <li key={link.label}>{renderLink(link)}</li>
          ))}
        </ul>
